...
with roots $r=\pm \sqrt{k/m}$. Thus the general solution is the following:
...
$\omega_0$ is the circular frequency, $R$ is the maximum displacement and the amplitude of the wave, and $\delta$ is the phase, or the offset of the wave from its natural position (eg offset from $0^\circ$)
...
$$
    r_1,r_2=\frac{-\gamma \pm \sqrt{\gamma^2-4km}}{2m} \space (\text{Quadratic})
$$
...
!!! example: Example 1
    ...
    $$
        \begin{aligned}
        r_1,r_2=\frac{-2 \pm \sqrt{2^2-4(1\cdot5)}}{2\cdot1}
        \end{aligned}
    $$
...
!!! example: Example 3
    ...
    $$
        \begin{matrix}
        r^2+\omega^2r = 0 \\
        r_1,r_2 =\frac{-\omega^2 \pm \sqrt{(\omega^2)^2-4(1\cdot0})}{2\cdot1}
        \end{matrix}
    $$
    $$
    ...
    **Case 1**: When $\omega \not= \omega_0$
    ...
    $$
        \begin{aligned}
        u_p &= \frac{F_0}{\omega_0^2-\omega^2}\cdot\cos(\omega t) + 0\cdot\sin(\omega t) \\
